 ...
 * 1. Removed the WindowEvent import/usage that was only there to build
 *    an unused WindowEvent object inside the old SystemExit() method.
 * 2. Removed the duplicated SystemExit() method - now uses the shared
 *    util.WindowUtils.closeWindow(), which previously was copy-pasted
 *    identically into 5 different files.
 * 3. Replaced hardcoded "1234"/"checkin"/"checkout" literals with named
 *    constants from util.AppConstants, so credentials live in ONE place.
 * 4. BUG FIX: this class previously had no package declaration (it sat
 *    in the default package) while every class that uses it (MainPage,
 *    display) lives in the "parking" package. A class in a named
 *    package cannot validly reference a default-package class in
 *    standard Java; this only appeared to work before because of
 *    NetBeans-specific build leniency, not portable javac behavior.
 *    Moving this class into the parking package (and removing the
 *    now-redundant CarOut/MainPage/display imports, since they are in
 *    the same package) fixes the real
 *    "cannot find symbol: class login" compile error.
 *
...
public class login extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        Register.setBorder(new javax.swing.border.SoftBevelBorder(javax.swing.border.BevelBorder.RAISED, java.awt.Color.gray, new java.awt.Color(204, 204, 0), java.awt.Color.gray, java.awt.Color.black));
    ...
    private void login1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_login1ActionPerformed
        ...
        // BUG FIX / FEATURE: staff logins used to be two permanently
        // hardcoded accounts ("checkin"/"checkout", both password
        // "1234"). They are now real accounts created via the
        // Register button below and stored in the database, so admin
        // can add, deactivate, or reset any staff member's password.
        String role = staffDb.validateLogin(name, pass);

        if (role != null && (role.equalsIgnoreCase("checkin") || role.equalsIgnoreCase("both"))) {
            jPassword.setText(null);
            Text.setText(null);
            WindowUtils.closeWindow(this);
            display info = new display();
            info.setVisible(true);
        } else if (role != null && role.equalsIgnoreCase("checkout")) {
            jPassword.setText(null);
            Text.setText(null);
            WindowUtils.closeWindow(this);
            ...
            JOptionPane.showMessageDialog(null,
                    "Invalid login details, or this account has been deactivated by the admin.",
    ...
    private void RegisterActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_RegisterActionPerformed
        WindowUtils.closeWindow(this);
        StaffRegister r1 = new StaffRegister();
        r1.setVisible(true);
